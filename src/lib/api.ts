...
export const ERROR_PLACEHOLDER_IMAGE =
...
export const fetchFonts = async (): Promise<FontCategory[]> => {
  ...
  if (!availableFonts) {
    throw new TypeError(
      "API response did not contain 'available_fonts' field.",
    );
  }
  ...
  const transformedData: FontCategory[] = Object.keys(categories).map(
    (name) => ({
      name,
